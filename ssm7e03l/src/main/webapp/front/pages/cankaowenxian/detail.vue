	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
				...
				<swiper-item v-for="(swiper,index) in swiperList" :key="index">
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>文献分类：</view>
					<view class="text" :style='{"color":"rgba(0, 0, 0, 1)","textAlign":"left","fontSize":"28rpx"}'>{{detail.wenxianfenlei}}</view>
				</view>
                				                								                								                				<view class="detail-list-item" :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","margin":"0 0 2rpx 0","borderColor":"rgba(171, 174, 176, 1)","borderRadius":"0","borderWidth":"2rpx","borderStyle":"none none solid none","height":"68rpx"}' @tap="download(detail.wenxianfujian)" >
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"rgba(6, 6, 6, 1)","textAlign":"left"}'>文献附件：</view>
				...
				<view class="content" :style='{"color":"#333","fontSize":"28rpx"}'>
					<rich-text style="font-size: 33upx;line-height: 50upx;letter-spacing: 5upx;" :nodes="detail.wenxianneirong"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','cankaowenxian');
						...
						if (res.confirm) {
							await _this.$api.add('storeup', {
								userid: _this.user.id,
								                												name: _this.detail.wenxianmingcheng,
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','cankaowenxian');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('cankaowenxian', this.id);
			...
			async upCallback(mescroll) {
				                let res = await this.$api.list('discusscankaowenxian', {
					page: mescroll.num,
					limit: mescroll.size,
					refid: this.id
				});
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusscankaowenxian/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('cankaowenxian', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('cankaowenxian', this.detail);
				this.$utils.msg('审核成功');
